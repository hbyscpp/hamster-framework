...
public abstract class BenchmarkClient {
	...
	//				"must give seven or eight args, serverIP serverPort concurrents timeout codectype requestSize runtime(seconds) clientNums");
		...
		runtime =30;// Integer.parseInt(args[6]);
		final long endtime = System.currentTimeMillis() + runtime * 1000;
		int tmpClientNums = 1;
		if (args.length == 8) {
			tmpClientNums = 2;//Integer.parseInt(args[7]);
		}
		...
		long benchmarkBeginTime = System.currentTimeMillis()+ 10000;
		for (int i = 0; i < concurrents; i++) {
			ClientRunnable runnable = getClientRunnable(serverIP, serverPort,
					clientNums, timeout, codectype, requestSize, barrier,
					latch, endtime, benchmarkBeginTime);
		...
		for (int i = 0; i < 10; i++) {
			...
			Long[] errorValues = errorTimes.remove(String.valueOf(maxTimeRange
					- i));
			if (errorValues != null) {
	...
	public abstract ClientRunnable getClientRunnable(String targetIP,
			int targetPort, int clientNums, int rpcTimeout, int codecType,
	...
	public static interface ClientRunnable extends Runnable {
