...
import com.seaky.hamster.core.benchmark.BenchmarkClient.ClientRunnable;

public class BenchmarkClientRunnable implements ClientRunnable {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(BenchmarkClientRunnable.class);
	...
	public BenchmarkClientRunnable(BenchmarkTestService testService,
	...
	private void runWork() {
		while (running) {
			...
			requestObject.setData(new byte[requestSize]);
			...
			try {
				ResponseObject response = testService.callMe(requestObject);
				...
				int range = Integer.parseInt(String.valueOf(beginTime
						- startTime)) / 1000;
				if (range >= maxRange) {
					// IGNORE
					continue;
				...
				if (response != null) {
					...
					errorResponseTimes[range] = errorResponseTimes[range]
							+ consumeTime;
				}
			} catch (Exception e) {
