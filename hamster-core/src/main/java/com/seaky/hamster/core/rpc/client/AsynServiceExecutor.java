...
public class AsynServiceExecutor<Req, Rsp> {
	...
	public void callService(ReferenceServiceRequest request,
			SettableFuture<Object> result, EndpointConfig config) {

		ServiceContext sc = new DefaultServiceContext(
		...
		ServiceContextUtils.setReferenceVersion(sc,
				request.getReferenceVersion());
		...
		boolean notUseThreadpoolConfig = config.getValueAsBoolean(
		...
		if (!notUseThreadpoolConfig) {
			...
			// 线程池执行
			String serviceThreadpoolName = config.get(
	...
	private static class CallRemoteServiceTask<Req, Rsp> implements Runnable {
		...
		private ClusterService<Req, Rsp> getClusterService(
			...
			String clusterName = ServiceContextUtils
					.getReferenceConfig(context).get(
		...
		private List<ServiceProviderDescriptor> chooseServiceInstances() {
			...
			Collection<ServiceProviderDescriptor> allServiceDescriptors = client
					.getAllServices(ServiceContextUtils.getServiceName(context));
		...
		private ServiceLoadBalancer getLoadBalancer() {
			String lbName = ServiceContextUtils.getReferenceConfig(context)
					.get(ConfigConstans.REFERENCE_LOADBALANCER,
							ConfigConstans.REFERENCE_LOADBALANCER_DEFAULT);
