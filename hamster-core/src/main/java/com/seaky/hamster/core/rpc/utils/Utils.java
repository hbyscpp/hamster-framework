...
public class Utils {
	...
	public static InetSocketAddress stringToSocketAddr(String addr) {
		...
		InetSocketAddress inetAddr = new InetSocketAddress(attrs[0],
				Integer.valueOf(attrs[1]));
		return inetAddr;
	}
	...
	public static boolean isMatch(String[] paramTypes, Object[] request) {
		...
		for (int i = 0; i < leftLength; ++i) {
			try {
				if (!isCompatibility(Class.forName(paramTypes[i]), request[i]))
					return false;
	...
	private static boolean isCompatibility(Class<?> type, Object obj) {
		if (type.isPrimitive()) {
			if (obj == null)
				return false;
			if (type == int.class)
	...
	public static boolean isVersionComp(String left, String right) {
		try {
			...
			int rightmajorversion = Integer.valueOf(right.substring(0,
					rightpos1));
			...
			int leftminorversion = Integer.valueOf(left.substring(leftpos1 + 1,
					leftpos2));

			int rightpos2 = right.indexOf(".", rightpos1 + 1);

			int rightminorversion = Integer.valueOf(right.substring(
					rightpos1 + 1, rightpos2));
			...
			if (rightpos3 == -1) {
				rightrevversion = Integer.valueOf(right
						.substring(rightpos2 + 1));
			} else {
				rightrevversion = Integer.valueOf(right.substring(
						rightpos2 + 1, rightpos3));
			}
	...
	public static void addDefaultInterceptor(List<ServiceInterceptor> sis,
			ProcessPhase phase) {
		String[] defaultNames = new String[] { "accesslog", "tracelog" };
		for (String name : defaultNames) {
			ServiceInterceptor si = ServiceInterceptorManager
					.createServiceInterceptor(name);
			ServiceInterceptorAnnotation anno = si.getClass().getAnnotation(
					ServiceInterceptorAnnotation.class);
	...
	public static List<ServiceInterceptor> extractByProcessPhase(
			String interceptors, ProcessPhase phase) {
		...
		for (int i = 0; i < size; ++i) {
			ServiceInterceptor si = ServiceInterceptorManager
					.createServiceInterceptor(interceptorNames[i]);
			if (si == null)
				throw new RuntimeException("not found service interceptor "
						+ interceptorNames[i]);
			ServiceInterceptorAnnotation anno = si.getClass().getAnnotation(
					ServiceInterceptorAnnotation.class);
			for (ProcessPhase p : anno.phases()) {
	...
	public static String createServerAndClientAddress(String clientHost,
			int clientPort, String serverHost, int serverPort) {
		StringBuilder sb = new StringBuilder();
