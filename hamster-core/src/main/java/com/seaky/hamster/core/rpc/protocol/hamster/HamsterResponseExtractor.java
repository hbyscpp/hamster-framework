...
public class HamsterResponseExtractor implements ResponseConvertor<HamsterResponse> {
  ...
  public Response convertFrom(HamsterResponse rsp) {
    ...
    try {
      ObjectHolder holder = ser.deSerialize(rsp.getResult(), ObjectHolder.class);
  ...
  public HamsterResponse convertTo(Response rsp) {
    ...
    Map<String, String> attachments = rsp.getAttachments().getAllAttachments();
    if (attachments != null) {
      for (Entry<String, String> entry : attachments.entrySet()) {
