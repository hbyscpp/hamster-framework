...
public class NettyClientTransport<Req, Rsp> implements
	...
	private void setFuture(ChannelFuture cf, SettableFuture<?> resultFuture,
		...
		if (cf.isSuccess()) {
			InetSocketAddress caddr = (InetSocketAddress) cf.channel()
					.localAddress();
			ServiceContextUtils.setClientHost(sc, caddr.getAddress()
					.getHostAddress());
	...
	private void setFutureListener(final ServiceContext sc, Req req,
		...
		final String seqNum = ServiceContextUtils.getRequestAttachments(sc)
		...
		writeFuture.addListener(new ChannelFutureListener() {
			...
			public void operationComplete(ChannelFuture future)
				...
				if (future.isSuccess()) {
					// 发送成功，设置异步超时
					int seconds = ServiceContextUtils
							.getReferenceConfig(sc)
							.getValueAsInt(
	...
	public void setResponse(Rsp rsp) {
		...
		Response response = protocolExtensionFactory.getResponseConvertor()
				.convertFrom(rsp);

		String seqNum = response.getAttachments().getAttachment(
		...
		if (response.getResult() instanceof ExceptionResult) {
			String expCon = ServiceContextUtils.getReferenceConfig(sc).get(
					ConfigConstans.REFERENCE_EXCEPTION_CONVERTOR, "default");
			ExceptionConvertor convert = exceptionConvertorExt
					.findExtension(expCon);
			RpcException e = convert.convertFrom(((ExceptionResult) response
					.getResult()));
