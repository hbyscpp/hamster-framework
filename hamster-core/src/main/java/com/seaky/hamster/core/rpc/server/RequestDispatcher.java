...
public class RequestDispatcher<Req, Rsp> {
	...
	static ExtensionLoader<ExceptionConvertor> exceptionConvertorExt = ExtensionLoader
			.getExtensionLoaders(ExceptionConvertor.class);
	...
	private static class ServiceRunner<Req, Rsp> implements Runnable {
		...
		public ServiceRunner(RequestDispatcher<Req, Rsp> dispatcher,
				...
				ServiceContext context, List<ServiceInterceptor> interceptors) {
			this.dispatcher = dispatcher;
		...
		public void run() {
			...
			if (service == null) {
				dispatcher.setException(context,
						new ServiceProviderNotFoundException(serviceName));
				dispatcher.writeResponse(context, transport);
				return;
			}

			EndpointConfig sc = ServiceContextUtils.getProviderConfig(context);
			String[] params = dispatcher.server.getServiceDescriptor(app,
					serviceName, serviceVersion, group).getParamTypes();
			if (!Utils.isMatch(params, context.getAttribute(
					ServiceContext.REQUEST_PARAMS, Object[].class))) {
				dispatcher.setException(context,
						new ServiceSigMismatchException(serviceName));
				dispatcher.writeResponse(context, transport);
				return;
			}
			// 1判断是否当前线程执行 还是线程池执行
			...
			ServiceRunner<Req, Rsp> sr = new ServiceRunner<Req, Rsp>(
					dispatcher, service, transport, context, interceptors);
	...
	private ServiceContext initContext(Req request,
		...
		ServiceContextUtils.setServerHost(context, transport.getLocalAddress()
				.getAddress().getHostAddress());
		ServiceContextUtils.setServerPort(context, transport.getLocalAddress()
				.getPort());

		ServiceContextUtils.setClientHost(context, transport.getRemoteAddress()
				.getAddress().getHostAddress());
		ServiceContextUtils.setClientPort(context, transport.getRemoteAddress()
				.getPort());
		ServiceContextUtils.setResponse(context, new Response());
		ServiceContextUtils.setRequestAttachments(context, new Attachments());

		try {
			server.getProtocolExtensionFactory().getRequestExtractor()
					.extractTo(request, context);
			// 验证所需的参数
			validateRequestInfo(context);
			// 用自身的配置
			EndpointConfig config = server.getServiceConfig(
					ServiceContextUtils.getApp(context),
					ServiceContextUtils.getServiceName(context),
					ServiceContextUtils.getVersion(context),
					ServiceContextUtils.getGroup(context));

			if (config == null) {
				throw new ServiceProviderConfigNotFoundException(
						ServiceContextUtils.getApp(context),
						ServiceContextUtils.getServiceName(context),
						ServiceContextUtils.getVersion(context),
						ServiceContextUtils.getGroup(context));
			}
			ServiceContextUtils.setProviderConfig(context,
					new ReadOnlyEndpointConfig(config));
		} catch (Exception e) {
	...
	private void writeResponse(ServiceContext sc,
		...
		try {
			...
			Attachments requestAttchments = ServiceContextUtils
					.getRequestAttachments(sc);

			Map<String, String> allAttachment = requestAttchments
					.getAllAttachments();
			if (allAttachment != null) {
				for (Entry<String, String> attach : allAttachment.entrySet()) {
					ServiceContextUtils.getResponse(sc).getAttachments()
							.addAttachment(attach.getKey(), attach.getValue());
				}
			}
			if (!response.isDone())
			...
			if (response.isException()) {
				...
				if (!(e instanceof RpcException)) {
					setException(sc, new UnknownException(e));

				}
			}
			ResponseConvertor<Rsp> attrWriter = server
					.getProtocolExtensionFactory().getResponseConvertor();
			Response r = ServiceContextUtils.getResponse(sc);
			if (r.isException()) {
				String expCon = ServiceContextUtils.getProviderConfig(sc).get(
						ConfigConstans.PROVIDER_EXCEPTION_CONVERTOR, "default");
				ExceptionConvertor convert = exceptionConvertorExt
						.findExtension(expCon);
