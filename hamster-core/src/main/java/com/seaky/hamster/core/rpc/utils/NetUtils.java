...
public class NetUtils {
  ...
  public static int getAvailablePort() {
    return getAvailablePort(RND_PORT_START, RND_PORT_END);
  }

  public static int getAvailablePort(int port) {
    return getAvailablePort(port, MAX_PORT);
  }


  public static int getAvailablePort(int port, int maxPort) {
    if (port <= 0) {
      throw new RuntimeException("min port must be greater than " + port);
    }
    if (maxPort > MAX_PORT)
    ...
    for (int i = port; i < maxPort; i++) {
      ...
      try {
        ss = new ServerSocket(i, 50, NetUtils.getLocalAddress());
        return i;
      } catch (IOException e) {
