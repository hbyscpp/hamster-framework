...
public class ServiceInterceptorManager {
	...
	private static ConcurrentHashMap<String, ServiceInterceptor> allInterceptors = new ConcurrentHashMap<String, ServiceInterceptor>();

	private static ConcurrentHashMap<String, Class<?>> allInterceptorsClass = new ConcurrentHashMap<String, Class<?>>();
	static {
		Reflections r = new Reflections(new Object[0]);
		Set<Class<?>> clss = r
				.getTypesAnnotatedWith(ServiceInterceptorAnnotation.class);
		...
		if (clss != null) {
			for (Class<?> cls : clss) {
				...
				ServiceInterceptorAnnotation anno = cls
						.getAnnotation(ServiceInterceptorAnnotation.class);
				...
				if (allInterceptorsClass.putIfAbsent(anno.name(), cls) != null) {
					throw new RuntimeException(anno.name()
							+ " service interceptor has regist in "
	...
	public static ServiceInterceptor createServiceInterceptor(String name) {
		...
		ServiceInterceptor si = allInterceptors.putIfAbsent(name,
				(ServiceInterceptor) obj);
		if (si == null)
